BINARY   := athenzctl
PKG      := github.com/fsul7o/athenzctl
CMD      := ./cmd/athenzctl
VERSION  ?= $(shell git describe --tags --always --dirty 2>/dev/null || echo dev)
COMMIT   ?= $(shell git rev-parse --short HEAD 2>/dev/null || echo none)
DATE     ?= $(shell date -u +%Y-%m-%dT%H:%M:%SZ)
LDFLAGS  := -s -w \
	-X $(PKG)/internal/version.Version=$(VERSION) \
	-X $(PKG)/internal/version.Commit=$(COMMIT) \
	-X $(PKG)/internal/version.Date=$(DATE) \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultDNSDomain=$(ISSUE_DEFAULT_SERVICECERT_DNS_DOMAIN)' \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultSubjectCountry=$(ISSUE_DEFAULT_SERVICECERT_SUBJ_C)' \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultSubjectProvince=$(ISSUE_DEFAULT_SERVICECERT_SUBJ_P)' \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultSubjectOrganization=$(ISSUE_DEFAULT_SERVICECERT_SUBJ_O)' \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultSubjectOrganizationalUnit=$(ISSUE_DEFAULT_SERVICECERT_SUBJ_OU)' \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultSpiffe=$(ISSUE_DEFAULT_SERVICECERT_SPIFFE)' \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultSpiffeTrustDomain=$(ISSUE_DEFAULT_SERVICECERT_SPIFFE_TRUST_DOMAIN)' \
	-X '$(PKG)/internal/authmode.ServiceCertDefaultConcatIntermediateCert=$(ISSUE_DEFAULT_SERVICECERT_CONCAT_INTERMEDIATE_CERT)' \
	-X '$(PKG)/internal/ntokenauth.NTokenHeaderDefault=$(NTOKEN_AUTH_DEFAULT_HDR)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultDNSDomain=$(ISSUE_DEFAULT_ROLECERT_DNS_DOMAIN)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultSubjectCountry=$(ISSUE_DEFAULT_ROLECERT_SUBJ_C)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultSubjectProvince=$(ISSUE_DEFAULT_ROLECERT_SUBJ_P)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultSubjectOrganization=$(ISSUE_DEFAULT_ROLECERT_SUBJ_O)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultSubjectOrganizationalUnit=$(ISSUE_DEFAULT_ROLECERT_SUBJ_OU)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultSpiffe=$(ISSUE_DEFAULT_ROLECERT_SPIFFE)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultSpiffeTrustDomain=$(ISSUE_DEFAULT_ROLECERT_SPIFFE_TRUST_DOMAIN)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultConcatIntermediateCert=$(ISSUE_DEFAULT_ROLECERT_CONCAT_INTERMEDIATE_CERT)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultCACertBundleName=$(ISSUE_DEFAULT_ROLECERT_CACERT_BUNDLE_NAME)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultExpiryTime=$(ISSUE_DEFAULT_ROLECERT_EXPIRY_TIME)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultIP=$(ISSUE_DEFAULT_ROLECERT_IP)' \
	-X '$(PKG)/internal/cli/issue.RoleCertDefaultSignerKeyID=$(ISSUE_DEFAULT_ROLECERT_SIGNER_KEY_ID)'

ATHENZ_DIST_DIR    := .local/athenz-distribution
ATHENZ_DIST_REPO   := https://github.com/ctyano/athenz-distribution.git
ATHENZ_DIST_PATCHES := $(wildcard $(PWD)/scripts/patches/athenz-distribution-*.patch)
E2E_CONFIG         := $(PWD)/.local/e2e/config.yaml
E2E_COVERAGE       ?= 1
ifeq ($(strip $(CI)),)
E2E_IMAGE_TARGETS  := deploy-kubernetes-in-docker load-docker-images load-kubernetes-images
else
E2E_IMAGE_TARGETS  := 
endif

.PHONY: build test lint tidy install clean snapshot \
	e2e-clone e2e-up e2e-down e2e e2e-focus e2e-sweep e2e-clean

build:
	go build -trimpath -ldflags "$(LDFLAGS)" -o bin/$(BINARY) $(CMD)

test:
	go test ./...

lint:
	go vet ./...

tidy:
	go mod tidy

install:
	go install -trimpath -ldflags "$(LDFLAGS)" $(CMD)

clean:
	rm -rf bin dist

# Vendored athenz-distribution is treated as disposable (see e2e-clean): any
# locally applied patches are reverted before pulling so `--ff-only` always
# succeeds, then re-applied on top of the fresh checkout every time.
e2e-clone:
	@if [ ! -d $(ATHENZ_DIST_DIR)/.git ]; then \
		mkdir -p $(dir $(ATHENZ_DIST_DIR)); \
		git clone --depth 1 $(ATHENZ_DIST_REPO) $(ATHENZ_DIST_DIR); \
	else \
		git -C $(ATHENZ_DIST_DIR) checkout -- .; \
		git -C $(ATHENZ_DIST_DIR) pull --ff-only; \
	fi
	@for p in $(ATHENZ_DIST_PATCHES); do \
		echo "applying $$p"; \
		git -C $(ATHENZ_DIST_DIR) apply "$$p" || exit 1; \
	done

e2e-up: e2e-clone
	$(MAKE) -C $(ATHENZ_DIST_DIR) \
		$(E2E_IMAGE_TARGETS) \
		deploy-kubernetes-crypki-softhsm use-kubernetes-crypki-softhsm \
		deploy-kubernetes-athenz check-kubernetes-athenz deploy-kubernetes-athenz-oauth2
	./scripts/e2e-bootstrap.sh $(ATHENZ_DIST_DIR) $(PWD)/.local/e2e

e2e-down:
	-./scripts/e2e-teardown.sh $(PWD)/.local/e2e
	-$(MAKE) -C $(ATHENZ_DIST_DIR) clean-kubernetes-athenz

e2e:
	ATHENZCTL_E2E_CONFIG=$(E2E_CONFIG) \
	ATHENZCTL_E2E_COVERAGE=$(E2E_COVERAGE) \
	GODOG_TAGS=$${GODOG_TAGS:-~@skip} \
	go test -tags=e2e -count=1 -v ./test/e2e/...

e2e-focus:
	GODOG_TAGS=$${GODOG_TAGS:-@focus} E2E_COVERAGE=0 $(MAKE) e2e

e2e-sweep:
	ATHENZCTL_E2E_CONFIG=$(E2E_CONFIG) \
	GODOG_TAGS=@__sweep_only__ \
	go test -tags=e2e -count=1 -v ./test/e2e/...

e2e-clean:
	rm -rf .local
